...
Решение [BE-A01 #52](BE_A01_RESULT.md); реализация — отдельный backend #53. Wire v1, его типы и три fixtures не меняются; quota/auth metadata не добавляются. Ниже — целевое поведение, не утверждение о готовности сервера. Клиентские ограничения остаются независимыми.
