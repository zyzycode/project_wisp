...
- Поля character/messages/locale/ID и весь HTTP/error dictionary имеют прежнюю семантику [v1 §1–3](BACKEND_API_CONTRACT.md#1-транспорт-и-владение), кроме version2. Request ≤32 KiB, response ≤16 KiB; UTF-16 text limits и exact-shape сохраняются. `requestId:null` допустим только для нераспознанного/невалидного ID в ошибке.
...
- Closed-network access, общие deployment budgets, UTC accounting, cancellation/deadline10s и retention остаются [v1 §5–6](BACKEND_API_CONTRACT.md#5-закрытая-alpha-серверный-admission-учёт-и-повторы). Ledger общий для обеих версий; digest включает method+path+точные body bytes. Повтор ID на другом endpoint →409 conflict, а не вторая бесплатная генерация. Для v1-only ledger прежний body digest допустим; при внедрении v2 legacy records нельзя replay через v2, их прежний TTL соблюдается.
...
Обе стороны используют **одни и те же** JSON-файлы из [desktop-backend-v2](../contracts/fixtures/desktop-backend-v2/): [request.valid](../contracts/fixtures/desktop-backend-v2/request.valid.json), [response.success](../contracts/fixtures/desktop-backend-v2/response.success.json), [response.error](../contracts/fixtures/desktop-backend-v2/response.error.json). Success/error — альтернативные HTTP200/503 outcomes. V1 fixtures остаются неизменными и продолжают проходить.
