...
Этот документ — единый журнал арт-долга между разработчиками (`app-developer`) и художником (`sprite-artist`).

## Правила для разработчика (`app-developer`)
1. Разработчик **не генерирует, не нарезает и не рисует** спрайты и **не изменяет файлы внутри `asset-pipeline/`**.
2. Если в задаче требуется анимация/спрайт, отсутствующий в `public/assets/sprites/`:
   ...
   - Указать ключ спрайта в строке отчёта `Sprites: <список>`.
