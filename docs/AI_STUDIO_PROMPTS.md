...
> **Цель:** Генерация четких, геометрически стабильных **4-кадровых промежуточных спрайт-листов (Keyframes)** персонажа Wisp в **горизонтальной сетке 1 строка × 4 колонки (1×4 horizontal grid)** по **прикреплённому референсу (Attached Reference Image)** в формате **PNG с прозрачным фоном (Transparent PNG with Alpha Channel)**.
> 
> *Сетка 1×4 из 4 ключевых кадров минимизирует «кипение» нейросети и сохраняет пропорции персонажа. Для проекта результат затем нарезается на отдельные PNG по структуре `docs/ARTIST_BRIEF.md`.*
...
## 📋 Промпты для генерации промежуточных sheets (4 ключевых кадра, сетка 1×4)
