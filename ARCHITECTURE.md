...
На схеме сплошные стрелки показывают путь вызова из интерфейса, пунктир — реализацию портов адаптерами. Обратная доставка состояния в UI описана ниже, в разделе IPC. Это не схема разрешённых импортов: точные границы определяют [архитектурные правила](.agents/rules/rules.md).
...
Технические требования к этим областям и безопасность окна собраны в [правилах разработки](.agents/rules/rules.md).
...
Правило размещения интерфейсов находится в [правилах разработки](.agents/rules/rules.md). Текущий интерфейс платформы доступен в [коде порта](src/application/ports/platform-adapter.interface.ts); его сигнатуры здесь не копируются.

### 5.3. Специфика Linux: X11 vs Wayland

На Linux имеет значение не только название ОС, но и тип графической сессии. Возможности глобального позиционирования и наблюдения за курсором могут различаться. Это объясняет наличие адаптеров и fallback-поведения; конкретные требования к X11 и Wayland приведены в [правилах разработки](.agents/rules/rules.md).
...
Preload предоставляет интерфейсу небольшой набор методов `window.wispAPI`. Для React-компонента это точка обращения к приложению; детали доставки запроса остаются за мостом. Требования к изоляции и экспорту методов находятся в [правилах разработки](.agents/rules/rules.md), реализация — в [preload](src/preload/index.ts).

### 6.3. Renderer Process (Слой отображения)

Renderer превращает полученное состояние в видимого персонажа, меню и диалоговые элементы. Здесь также живут локальные состояния интерфейса: например, введённый текст или раскрытое меню. Разделение UI-состояния, состояния приложения и компонентов описано в [правилах разработки](.agents/rules/rules.md).
...
В Electron для запросов используются пары `invoke` / `handle`, для событий из Main — `webContents.send`. Это объяснение механизма, а не перечень доступных методов Wisp. Реализованные DTO и мост определены в [src/shared/ipc-contracts.ts](src/shared/ipc-contracts.ts), требования к безопасному preload и сериализации — в [правилах разработки](.agents/rules/rules.md).
...
Следующие описания поясняют роли слоёв. Правила зависимостей, размещения портов и изоляции задаёт [rules.md](.agents/rules/rules.md).

### 8.1. UI Layer (Renderer)

UI отвечает на вопрос «что сейчас показать пользователю». Для него важны состояние отображения и события взаимодействия. Выбор визуального ресурса и композиция персонажа раскрываются в [Render Engine](docs/engine/RENDER_ENGINE.md), приёмы разработки интерфейса — в [правилах разработки](.agents/rules/rules.md).
...
Выбрать источник помогает [общий навигатор](docs/README.md); локальный указатель находится в [docs/engine/README.md](docs/engine/README.md), причины решений — в [ADR index](docs/adr/README.md). Принятый контракт UI/Renderer находится в [UI_SPEC.md](docs/engine/UI_SPEC.md). Порядок изменения контрактов установлен [правилами разработки](.agents/rules/rules.md).
...
Целевая схема SQLite, миграции, DTO и очистка памяти описаны в [MEMORY_ENGINE.md](docs/engine/MEMORY_ENGINE.md). Общие правила persistence и системных путей находятся в [правилах разработки](.agents/rules/rules.md). Имя файла базы и конкретные пути не задаются этим обзором. Готовность persistence отслеживается в [ROADMAP.md](ROADMAP.md).
...
Настройки безопасности окна, preload и навигации определяют [правила разработки](.agents/rules/rules.md). Правила доступа к данным и их очистки находятся в [контракте памяти](docs/engine/MEMORY_ENGINE.md). Флаги безопасности и списки запретов здесь не дублируются.

## 14. Матрица изоляции зависимостей

Актуальные ограничения зависимостей, безопасности и платформы находятся в [правилах разработки](.agents/rules/rules.md) и соответствующем [engine contract](docs/engine/README.md).
