...
import { calculateRootCollisionRange } from './motion-engine';

export interface RestSpotPlan { readonly target: ExplorePlan; readonly seated: boolean; readonly nap: boolean; readonly route: readonly ActivityStep[] }
export function selectRestSpot(context: ExplorePlanningContext, nap: boolean): RestSpotPlan | null {
  ...
  const ranked = scoreExplorePlans(candidates.map(plan => ({ ...plan, choreographyKey: `rest:${plan.surfaceKind}:${nap ? 'nap' : 'sleep'}` })), context).map(({ plan, weight }) => {
    const route = createExternalRoute(plan, context);
    const tier = plan.surfaceKind === 'window_top' ? 3 : plan.pointKind === 'edge' || plan.pointKind === 'corner' ? 2 : 1;
  ...
  const range = calculateRootCollisionRange(context.environment.screenBounds, context.collisionInsets);
  const target = best.plan;
  const width = target.targetSurface?.bounds.width ?? range.maxX - range.minX;
  const seated = target.surfaceKind === 'window_top' && (width < 240 || target.pointKind === 'edge' || target.pointKind === 'corner');
  return { target, seated, nap, route: best.route! };
}

export function createRestSpotActivity(plan: RestSpotPlan): ActivityDefinition {
  const phase = (id: string, kind: 'sit_edge' | 'sit' | 'lie_down' | 'sleep_start' | 'sleep_loop' | 'wake_up' | 'stand_up', durationMs: number, next?: string): ActivityStep => ({
  ...
  const prepare = plan.target.surfaceKind === 'window_top' ? 'sit_edge' : 'lie_down';
  const steps: ActivityStep[] = [
    ...plan.route.map(s => s.next === 'inspect' ? { ...s, next: 'prepare' } : s),
    phase('prepare', prepare, 1200, 'settle'),
    phase('settle', plan.seated ? 'sit_edge' : 'lie_down', 1500, 'sleep'),
    phase('sleep', plan.seated ? 'sit_edge' : 'sleep_loop', plan.nap ? 12_000 : 3000, plan.nap ? 'wake' : undefined),
  ];
  if (plan.nap) steps.push(phase('wake', plan.seated ? 'stand_up' : 'wake_up', 1500));
