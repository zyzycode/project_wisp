...
- публикует результат архитектурной задачи в назначенной Issue и передаёт задачу на `reviewer` gate (при изменении `docs/engine/*`, правил или портов), либо объявляет `done` (если задача чисто консультационная/без правок контрактов);
...
Если запись в GitHub недоступна, Architect возвращает готовый текст комментария пользователю и явно указывает этот blocker. Он не утверждает, что решение опубликовано.
...
`reviewer` (если менялись контракты docs/engine/*, правила или порты), `done` (если решение зафиксировано в Issue и не требует отдельного аудита) либо `blocked` с точной причиной. Менеджер проекта не вызывается внутри цикла задачи.
```
