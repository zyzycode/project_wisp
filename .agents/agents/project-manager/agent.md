...
- **Labels:** ровно один `owner:*`; существующие `phase:*`, `track:*`, `area:*`, `documentation` и `needs:architect` добавлять только по факту. Новые labels не придумывать без отдельной необходимости. Метку `documentation` НЕ ставить на архитектурные задачи, объявляющие контракты в коде (`src/application/ports/`, `src/shared/ipc-contracts.ts`), чтобы не вводить в заблуждение ревьюера; использовать только для чистых README/docs-only задач.
...
- <что точно не делаем в этой задаче (для архитектора: реализация бизнес-логики и UI — зона app-developer; не писать docs-only, если архитектор объявляет порты/DTO в src/)>
