...
| **Phase 7** | Interaction | 🟡 Следующая | Реакция на клики, наведение курсора, контекстное меню |
...
- **Что входит:**
  - Доменная логика автономных решений (`decideNextAutonomousAction`), расчёт случайных безопасных траекторий блуждания (`calculateNextWanderTarget`) и плавная интерполяция с плавным замедлением (`interpolatePosition`) в `src/domain/behavior/autonomous-behavior.ts`.
  - Хук автономности `useAutonomousBehavior` в `src/renderer/hooks/useAutonomousBehavior.ts`, управляющий тиками блуждания, переходом в режим парения (`START_FLOAT` $\rightarrow$ `STOP_FLOAT`), сном и пробуждением.
  - Возможность включения/выключения автономных прогулок и ручного усыпления/пробуждения через меню питомца.
  - Unit-тесты расчёта траекторий и вероятностных переходов (`tests/domain/autonomous-behavior.test.ts`).
- **Критерии готовности:**
