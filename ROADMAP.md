...
| **Phase 8** | Local Chat UI | 🟡 Следующая | Всплывающее окно сообщений/мыслей, поле ввода |
...
- **Что входит:**
  - Доменная логика жестов и уровня привязанности (`recordPetInteraction`, `calculateAffectionDecay`, `PetAffectionState`) в `src/domain/interaction/pet-interaction.ts`.
  - Поддержка одиночного клика (поглаживание/эмоция), двойного клика (быстрое открытие меню) и правого клика (контекстное меню).
  - Компонент `ContextMenu` в `src/renderer/components/Interaction/ContextMenu.tsx` с баром настроения, быстрыми действиями, палитрой тем, масштабом и выходом.
  - Unit-тесты расчёта привязанности и настроения (`tests/domain/pet-interaction.test.ts`).
- **Критерии готовности:**
